...
#ifndef BUZZSDK_SWIFT_H
#define BUZZSDK_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="BuzzSDK",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
/// Provides the product identifiers for in app purchases
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSArray<NSString *> * _Nonnull productIdentifiers;)
+ (NSArray<NSString *> * _Nonnull)productIdentifiers SWIFT_WARN_UNUSED_RESULT;
...
/// Indicates if a info button for data privacy and impressum should be presented next to card counter.
+ (BOOL)presentDataPrivacyInfoButton SWIFT_WARN_UNUSED_RESULT;
...
+ (NSString * _Nonnull)fontNameForInfoButtonLabelInNavigationView SWIFT_WARN_UNUSED_RESULT;
...
/// The Layout Coordinates defining the Title Content Paddings
- (UIEdgeInsets)titlePaddings SWIFT_WARN_UNUSED_RESULT;
- (LayoutCoordinatesObjC * _Nonnull)titleMarginsCompensationsIfNoKickerPresent SWIFT_WARN_UNUSED_RESULT;
- (CGFloat)titleIntraLineSeparation SWIFT_WARN_UNUSED_RESULT;
...
/// Defines if the kicker text shdould be allways be forced to upper case
- (BOOL)forceKickerUppercase SWIFT_WARN_UNUSED_RESULT;
...
/// Will return the list of UIViews considered friendly obstructions to the video content. i.e. The control buttons, the progress bar and the Load more button.
- (NSArray<UIView *> * _Nonnull)friendlyObstructions SWIFT_WARN_UNUSED_RESULT;
...
/// Notifies the delegate user request to skip playhead for playing video, and asks is it should allow it. If <code>froward</code> is <code>false</code> the user attemps rewind, otherwise is forward.
- (BOOL)fullScreenVideoPlayerController:(MFFullScreenVideoPlayerController * _Nonnull)controller shouldAllowUserToSkipForPlayer:(GDMediaPlayer * _Nonnull)mediaPlayer forward:(BOOL)forward SWIFT_WARN_UNUSED_RESULT;
/// Notifies the delegate the controller will move the playhead of the video to the given time. If delegate returns <code>false</code>, then controller will not seek the video player directly but will assume the delegate has done it and expects the <code>completion</code> block to be called when done. If delegate returns <code>true</code> the controller will take care of the playhead change and the <code>completion</code> block is ignored.
- (BOOL)fullScreenVideoPlayerController:(MFFullScreenVideoPlayerController * _Nonnull)controller willSeek:(GDMediaPlayer * _Nonnull)mediaPlayer to:(CMTime)time externalCompletion:(SWIFT_NOESCAPE void (^ _Nonnull)(BOOL))completion SWIFT_WARN_UNUSED_RESULT;
@optional
...
@interface MFNetworking (SWIFT_EXTENSION(BuzzSDK)) <NSURLSessionTaskDelegate>
- (void)URLSession:(NSURLSession * _Nonnull)session task:(NSURLSessionTask * _Nonnull)task didCompleteWithError:(NSError * _Nullable)error;
@end


@interface MFNetworking (SWIFT_EXTENSION(BuzzSDK)) <NSURLSessionDelegate>
- (void)URLSessionDidFinishEventsForBackgroundURLSession:(NSURLSession * _Nonnull)session;
@end


@interface MFNetworking (SWIFT_EXTENSION(BuzzSDK)) <NSURLSessionDownloadDelegate>
- (void)URLSession:(NSURLSession * _Nonnull)session downloadTask:(NSURLSessionDownloadTask * _Nonnull)downloadTask didFinishDownloadingToURL:(NSURL * _Nonnull)location;
@end


@interface MFNetworking (SWIFT_EXTENSION(BuzzSDK)) <NSURLSessionDataDelegate>
...
enum MFVideoAdDisplayType : NSInteger;
...
///
/// \endcode
- (void)processVideoAdFrom:(NSString * _Nonnull)vastURLString;
...
/// The skip offset for the ad, if available, otherwise -1
...
- (void)stop;
/// Pause Video Ad Playback if playing and report <code>skip</code> event
- (void)skip;
/// Pause Video Ad Playback if playing. <code>userAction</code> indicates if this is an explicit pause request by user. In such case event must be triggered.
- (void)pauseByUserAction:(BOOL)userAction;
...
/// Move video playhead to a specified time. Only performed for Rewind operations
- (void)seekTo:(CMTime)time completion:(void (^ _Nonnull)(BOOL))completion;
...
/// Notify the controller about display size change including friendly obstructions
- (void)displayTypeChanged:(enum MFVideoAdDisplayType)newType;
/// Notify the controller about changes on friendly obstructions
- (void)changedFriendlyObstructions:(NSArray<UIView *> * _Nullable)friendlyObstructions;
/// Call this method to indicate the controller that the Ad View has been tapped. This will handle clicks and tracking
- (void)tapped;
/// Indicates if the VAST video ad is to be considered as an autoplay video ad for tracking purposes. Default value is <code>false</code> and must be set to <code>true</code> before calling <code>play()</code> for tracking to correcly mark it as such.
@property (nonatomic) BOOL isAutoplay;
/// If GDPR consent and info should be including in tracking, then this property must be set to the correct <code>gdprConsentInfo</code> as soon as possible.
@property (nonatomic, copy) NSString * _Nullable gdprConsentInfo;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
- (void)mediaPlayer:(GDMediaPlayer * _Nonnull)mediaPlayer assetsMetadataLoadedForContentType:(GDMediaPlayerContentType)mediaPlayerContentType;
...
- (void)mediaPlayer:(GDMediaPlayer * _Nonnull)mediaPlayer masterPlayerLayerIsVisibleIn:(CALayer * _Nullable)superLayer;
- (void)mediaPlayer:(GDMediaPlayer * _Nonnull)mediaPlayer startedPlaybackForContentType:(GDMediaPlayerContentType)mediaPlayerContentType;
- (void)mediaPlayer:(GDMediaPlayer * _Nonnull)mediaPlayer playbackWasPausedForContentType:(GDMediaPlayerContentType)mediaPlayerContentType;
- (void)mediaPlayer:(GDMediaPlayer * _Nonnull)mediaPlayer isReadyForDisplayForContentType:(GDMediaPlayerContentType)mediaPlayerContentType;
- (void)mediaPlayer:(GDMediaPlayer * _Nonnull)mediaPlayer videoHasStalledForContentType:(GDMediaPlayerContentType)mediaPlayerContentType;
...
/// Reports when the <code>controller</code> video ad has finish loading assets for playback.
- (void)vastPlayerControllerHasLoadedVideoAssets:(MFVASTPlayerController * _Nonnull)controller;
...
- (nonnull instancetype)initWithURL:(NSURL * _Nonnull)url mimeType:(NSString * _Nullable)mimeType showToolbar:(BOOL)showToolbar OBJC_DESIGNATED_INITIALIZER;
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
@property (nonatomic, readonly) UIStatusBarStyle preferredStatusBarStyle;
- (void)viewDidLoad;
